...
require "../config/conexion.php";
	...
	Class Asignacion{
		...
		public function listar($idempl){
			$sql="SELECT a.idasignacion, a.created_time, d.marca, d.nombre as equipo, e.imei, c.numero, 
						o.nombre as operador, w.nombre, w.apellido, w.num_documento, f.nombre as cargo, 
						a.condicion, a.contrato, a.acta, a.devolucion, e.idequipo, c.idchip,
						IF(a.tasignacion=0,'NUEVA ASIGNACION',IF(a.tasignacion=1,'REPOSICION','')) as tasignacion 
						FROM asignacion a 
						INNER JOIN equipo e ON a.idequipo = e.idequipo 
						INNER JOIN detalle d ON e.iddetalle=d.iddetalle 
						INNER JOIN empleado w ON a.idempleado = w.idempleado 
						INNER JOIN cargos f ON w.idcargo=f.idcargos 
						INNER JOIN chip c ON a.idchip = c.idchip 
						INNER JOIN operador o on c.idoperador= o.idoperador 
						WHERE a.idempleado = $idempl";
			return ejecutarConsulta($sql);
		}
