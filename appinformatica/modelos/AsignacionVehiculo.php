...
require "../config/conexion.php";
	...
	Class AsignacionVehiculo{
		...
		public function listar(){
			...
			$sql="SELECT a.idasigvehi, r.*, DAY(r.fecha) AS dia, MONTH(r.fecha) AS mes, YEAR(r.fecha) AS año, v.ano, v.patente, m.nombre as modelo, n.nombre as marca, e.nombre, e.apellido, e.num_documento, c.nombre AS cargo, d.nombre AS departamento, u.nombre AS nomuser, u.apellido AS apeuser, u.num_documento AS numuser FROM asigvehi a INNER JOIN vehiculo v ON a.idvehiculo=v.idvehiculo INNER JOIN marcave n ON v.idmarca=n.idmarca INNER JOIN modelove m ON v.idmodelo=m.idmodelove INNER JOIN empleado e ON a.idempleado = e.idempleado INNER JOIN cargos c ON e.idcargo=c.idcargos INNER JOIN departamento d ON c.iddepartamento=d.iddepartamento INNER JOIN user u ON a.created_user = u.iduser INNER JOIN revision r ON r.idasigvehi=a.idasigvehi WHERE r.tipo='$tipo' AND a.idasigvehi='$idasigvehi'";
			return ejecutarConsultaSimpleFila($sql);
		}
