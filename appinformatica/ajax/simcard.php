...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()){
			...
			$data[] = array(
					"0"=>($reg->condicion)?
					'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idsimcard.')"><i class="fa fa-pencil"></i></button>'.
					'<button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idsimcard.')"><i class="fa fa-close"></i></button>':
					'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idsimcard.')"><i class="fa fa-pencil"></i></button>'.
					'<button class="btn btn-primary btn-xs" onclick="activar('.$reg->idsimcard.')"><i class="fa fa-check"></i></button>',
					"1"=>$reg->nombre,
					"2"=>$reg->numero,
					"3"=>$reg->serial,
