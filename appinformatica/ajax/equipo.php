...
switch ($_GET["op"]) {
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()){
			...
			$data[] = array(
					"0"=>($reg->condicion)?
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->idequipo.')"><i class="fa fa-pencil"></i></button>'.
					' <button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idequipo.')"><i class="fa fa-close"></i></button>':
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->idequipo.')"><i class="fa fa-pencil"></i></button>'.
