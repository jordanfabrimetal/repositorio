...
switch ($_GET["op"]) {
	...
	case 'guardaryeditar':
		...
		if(empty($idcomputador)){
			$rspta=$computador->insertar($idmarca, $modelo, $tcomputador, $serial, $maclan, $macwifi,$factura,$fvencimiento_garantia, $observaciones, $estado);
	...
	case 'listar':
		...
		while ($reg = $rspta->fetch_object()){
			$data[] = array(
					"0"=>($reg->condicion)?
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->idcomputador.')"><i class="fa fa-pencil"></i></button>'.
					' <button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idcomputador.')"><i class="fa fa-close"></i></button>':
					'<button class="btn btn-warning btn-xs" onclick="mostar('.$reg->idcomputador.')"><i class="fa fa-pencil"></i></button>'.
					' <button class="btn btn-primary btn-xs" onclick="activar('.$reg->idcomputador.')"><i class="fa fa-check"></i></button>',
					"1"=>$reg->tipo,
