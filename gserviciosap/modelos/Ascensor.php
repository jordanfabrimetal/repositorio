...
require_once "../config/conexionSap.php";
	...
	Class Ascensor{
        ...
        public function SelectTipoServicio(){
			...
			return $rspta = json_decode(Query($query), true);
        ...
        public function MostrarAudioriaTecnica($fm){
        	/* ver si existe alguno abierto o antendido*/
        	$sql = "ServiceCalls/\$count?\$filter=Status eq -3 and InternalSerialNum eq '".$fm."' and CallType eq 13";
        	$contador = Query($sql);
        	if($contador >= 1){
	        	/* Inicio ServicaCalls */
	        	$entity = 'ServiceCalls';
	        	$select = 'ServiceCallID,ItemCode,InternalSerialNum,CallType,Subject,CustomerCode';
	        	$filter = "Status eq -3 and InternalSerialNum eq '".$fm."' and CallType eq 13";
	        	$servcall = json_decode(ConsultaEntity($entity,$select,$filter), true);
	        	$servcall = $servcall['value'][0];
	        	//print_r($servcall);die;
				/* Fin ServicaCalls */
	        	...
	        	return json_encode(
	    			array(
	    				...
	    				"edificio"=>$nombre,
	    				"direccion"=>$direccion,
	    				"CallType"=>"Auditoria Tecnica",
	    				"CallTypeID"=>13,
	    				"status"=>$status,
	    				"Subject"=>"Visita generada por Integració",
	    				"InternalSerialNum"=>$InternalSerialNum,
	    				"modelo"=>$modelo,
	    				"tipoequipo"=>$tipoequipo,
	    				"nomenclatura"=>$nomenclatura,
        ...
        public function IniciarServicioEmergencia($observacionini,$fm,$customercode,$itemcode,$status,$gps){
			//$mifecha= date('Y-m-d H:i:s'); 
			//$NuevaFecha = strtotime ( '-4 hour' , strtotime ($mifecha) ) ; 
        ...
        public function IniciarServicioVisita($fm,$customercode,$itemcode,$status,$gps,$subject){
			//$mifecha= date('Y-m-d H:i:s'); 
			//$NuevaFecha = strtotime ( '-4 hour' , strtotime ($mifecha) ) ; 
        ...
        public function IniciarServicioAuditoriaTecnica($observacionini,$fm,$customercode,$itemcode,$status,$gps){
			//$mifecha= date('Y-m-d H:i:s'); 
			//$NuevaFecha = strtotime ( '-4 hour' , strtotime ($mifecha) ) ; 
